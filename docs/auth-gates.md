...
| `/preview` (POST — run), `/preview/{token}` (GET — poll) | POST/GET | **editor\*** | code-execution surface (spec preview-run). POST rides the verb-gate AND a HARD in-handler editor check; the GET is editor-gated in-handler too (`!principal.CanWrite` → 403), because a preview trace is the uploaded spec's output. Bounded (per-user rate + concurrency + hard timeout) and AUDITED (`sandbox_preview` + `audit_log`) from day one. Runs in the low-priv `synthwatch-sandbox` job, never at API/runner privilege. |
