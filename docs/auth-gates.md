...
| `/preview` (POST — run), `/preview/{token}` (GET — poll), `/preview/quota` (GET — bounds), `/preview/{token}/screenshot` (GET), `/preview/{token}/trace` (GET) | POST/GET | **editor\*** | code-execution surface (spec preview-run). POST rides the verb-gate AND a HARD in-handler editor check; the GETs are editor-gated in-handler too (`!principal.CanWrite` → 403), because a preview trace/screenshot/trace.zip is the uploaded spec's output and the quota is the caller's own usage. The screenshot/trace GETs STREAM the blob through the API (private container, MI-read, no SAS ⇒ no widening). Bounded (per-user rate + concurrency + hard timeout) and AUDITED (`sandbox_preview` + `audit_log`) from day one. Runs in the low-priv `synthwatch-sandbox` job, never at API/runner privilege. |
